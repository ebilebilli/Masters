...
class CreateReviewAPIView(APIView):
    ...
    def post(self, request, master_id):
        ...
        try:
            master = CustomPagination.objects.get(is_active=True, id=master_id)
        except CustomPagination.DoesNotExist:
            return Response({'detail': 'Usta tapılmadı.'}, status=status.HTTP_404_NOT_FOUND)
...
class FilterReviewAPIView(APIView):
    ...
    def get(self, request, master_id):
        pagination = self.pagination_class()
        master = get_object_or_404(CustomPagination, is_active=True, id=master_id)
